...
class _AddStudentScreenState extends State<AddStudentScreen> {
  ...
  @override
  void dispose() {
    for (final c in [_nameCtrl, _emailCtrl, _phoneCtrl, _pwCtrl, _semCtrl, _deptCtrl, _instCtrl]) {
      c.dispose();
    }
    super.dispose();
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      await StudentsService().createStudent(CreateStudentRequest(
        name: _nameCtrl.text.trim(),
        email: _emailCtrl.text.trim(),
        phone: _phoneCtrl.text.trim(),
        password: _pwCtrl.text.trim(),
        semesterId: int.parse(_semCtrl.text.trim()),
        departmentId: int.parse(_deptCtrl.text.trim()),
        institutionId: int.parse(_instCtrl.text.trim()),
      ));
      if (mounted) {
            ...
            child: Form(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 14),
                  AppTextField(label: 'Semester ID', controller: _semCtrl,
                  ...
                  AppTextField(label: 'Department ID', controller: _deptCtrl,
                      ...
                      validator: (v) => int.tryParse(v ?? '') == null ? 'Number required' : null),
                  const SizedBox(height: 14),
                  AppTextField(label: 'Institution ID', controller: _instCtrl,
                      keyboardType: TextInputType.number,
