...
func (h *Handler) ReadByID(c *gin.Context) {
	...
	log.Info().
		Str("component", "students_handler").
		Str("id", idStr).
		Msg("Received request to read student by ID")
	...
	response, cacheHit, err := h.service.GetStudentByID(c.Request.Context(), id)
	if err != nil {
		c.Error(err)
		return
	}
	...
	if cacheHit {
		log.Info().
			...
			Str("cache_key", fmt.Sprintf("student:v1:%d", id)).
			Msg("cache hit")
	} else {
		log.Info().
