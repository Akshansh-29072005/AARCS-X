...
func (h *Handler) ReadByID(c *gin.Context) {
	...
	log.Info().
		Str("component", "subjects_handler").
		Str("id", idStr).
		Msg("Received request to read subject by ID")
	...
	response, cacheHit, err := h.service.GetSubjectByID(c.Request.Context(), id)
	if err != nil {
		c.Error(err)
		return
	}
	...
	if cacheHit {
		log.Info().
			...
			Str("cache_key", fmt.Sprintf("subject:v1:%d", id)).
			Msg("cache hit")
	} else {
		log.Info().
